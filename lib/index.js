...
const ServiceAgent = class extends HttpAgent {
    ...
    addRequest(req, options, ...extra) {
        ...
        if (srvRecord in this.mappedRecord && srvRecord in this.resolvedTimeMap && this.intervalMilliSecs > 0) {

            if (Date.now() - this.resolvedTimeMap[srvRecord] >= this.intervalMilliSecs && this.resolvingMap.hasOwnProperty(srvRecord)) {
