...
const ServiceAgent = class extends HttpAgent {
    intervalSeconds = 5 * 1000; // set default interval to 5 seconds.
    service = '_http._tcp.';
    logger = null;
    ...
    constructor(options) {
        ...
        if (options && options.hasOwnProperty('intervalSeconds')) {
            ...
            if (typeof options.service !== 'number') {
                throw new TypeError('Service option must be a number');
            }

            this.intervalSeconds = options.intervalSeconds * 1000; // convert milliSec to sec.
        }
    ...
    addRequest(req, options, ...extra) {
        ...
        const sentRequest = () => {
            ...
            const addr = internals.groupSrvRecords(mappedRecord[srvRecord]).shift();
        ...
        if (srvRecord in mappedRecord && srvRecord in resolvedTimeMap && Date.now() - resolvedTimeMap[srvRecord] < this.intervalSeconds) {
            return sentRequest();
        }
        ...
        Dns.resolveSrv(srvRecord, (err, addrs) => {
            ...
            mappedRecord = Object.assign(mappedRecord, { [srvRecord]: addrs });
            resolvedTimeMap[srvRecord] = Date.now();
            return sentRequest();
        });
    }
};
