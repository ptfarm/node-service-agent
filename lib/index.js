...
const ServiceAgent = class extends HttpAgent {
    intervalMilliSecs = 5 * 1000; // set default interval to 5 seconds.
    service = '_http._tcp.';
    logger = null;
    ...
    addRequest(req, options, ...extra) {
        ...
        if (srvRecord in mappedRecord && srvRecord in resolvedTimeMap && Date.now() - resolvedTimeMap[srvRecord] < this.intervalMilliSecs) {
            return sentRequest();
        }
